-- Migration: server-side username moderation for leaderboards.
-- Safe to re-run. Adds a banned-words check inside submit_score (the only write
-- path, so it can't be bypassed by calling the API directly), plus a `hidden`
-- flag so anything that slips through can be removed.

-- 1. Banned words (normalised: lowercase letters only). Add/remove with INSERT/DELETE.
...
alter table public.banned_words enable row level security;  -- no policies = not client-readable

-- 2. Moderation flag + read policy that hides flagged rows from the public.
...
-- 3. Normalise a name for matching: lowercase, undo common leetspeak, drop anything
--    that isn't a letter (spaces, underscores, symbols), so "x_F.u.c.k_x" -> "fuck".
create or replace function public.normalize_name(p text)
returns text language sql immutable as $$
  select regexp_replace(
           translate(lower(p), '@$013457|!', 'asoieastli'),
           '[^a-z]', '', 'g')
$$;

-- 4. Rebuild submit_score with the banned-word check.
...
-- To remove something that slips through, in the SQL editor:
--   update public.scores set hidden = true where id = '<row id>';
