  ...
  return (
    <Modal isOpen={showUpgradeModal} onClose={closeUpgradeModal}>
      <div className="p-6">
            ...
            >
              ...
              <div className="mb-3">
                ...
                <div className="flex items-baseline gap-0.5 mt-1">
                  ...
                  <span className="text-white/65 text-xs">{tier.period}</span>
                </div>
              </div>
              <ul className="space-y-1.5 mb-4">
                {tier.features.map((f) => (
                  <li key={f} className="flex items-start gap-1.5 text-xs text-white/65">
                    <svg className="w-3 h-3 text-white/65 mt-0.5 flex-shrink-0" viewBox="0 0 12 12" fill="none">
        ...
        <button
          onClick={closeUpgradeModal}
          className="w-full text-center text-white/65 text-xs hover:text-white/70 transition-colors py-1 cursor-pointer"
        >
